...
public class Nivell1 {
    public static void main(String[] args) {
        ...
        int bitllet500, bitllet200, bitllet100, bitllet50, bitllet20, bitllet10,
                bitllet5, moneda2, moneda1, preuTotal = 0;
        String[] plats = new String[10];
        int[] preus = new int[10];
        //FASE 2. Omplim els arrays mitjançant un HashMap, mostrem els arrays i preguntem que volen menjar.
        omplirArrays(plats,preus);
        mostrarArrays(plats, preus);
        ArrayList<String> platsDemanats = demanarPlats();
    ...
    public static ArrayList<String> demanarPlats(){
        int opcioDemanar;
    ...
    public static int preguntarUsuari(){
        ...
        int opcio = -1;
        String missatgeError = "Error.Introdueix una opció vàlida.";
        while (preguntar){
            System.out.print("Vols seguir demanant plats?");
            System.out.println("Introdueix 1:Si / 0:No");
            if (sc.hasNextInt()){
                opcio = sc.nextInt();
                if (opcio == 1 || opcio == 0) preguntar = false;
                else {
    ...
    public static int calcularPreu(String[] arrayPlats, int[] arrayPreus, ArrayList<String> llistaPlatsDemanats){
        ...
        for (int i = 0; i < llistaPlatsDemanats.size(); i++){
            String nomPlat = llistaPlatsDemanats.get(i);
            int filaPlatArray = registrePlat(nomPlat, arrayPlats);
            if (filaPlatArray < arrayPlats.length) preuComanda += arrayPreus[filaPlatArray];
            else System.out.println("El plat " + nomPlat + " que has demanat no existeix.");
        }
        return preuComanda;
    }
